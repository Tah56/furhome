...
const AllPets = async ({search}) => {
  ...
  return (
    <div>
      ...
      ">
        {datas.map((pet) => {
          return (<div key={pet._id} className="w-full p-5">
  <Card className="cursor-pointer w-full overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 rounded-3xl group">
    ...
    <div className="relative">
      ...
      {/* Available Badge */}
      <div className={`absolute top-3 left-3  ${pet.status==="Accept"?'bg-red-100 text-red-700':"bg-green-100 text-green-700"}  text-xs font-medium px-3 py-1 rounded-full`}>
    ...
    <div className="p-4">
      ...
      <div className="flex items-center gap-4 mt-3 text-sm text-gray-600">
        <div className="flex items-center gap-1">
          {pet.gender === "Male" ? "♂" : "♀"} {pet.gender || pet.Species}
        </div>
        <div className="flex items-center gap-1">
          📅 {pet.age || "N/A"}
        </div>
      ...
      <p className="text-sm text-gray-600 mt-3 line-clamp-2">
        {pet.description || "Friendly and loving pet looking for a forever home."}
      </p>
